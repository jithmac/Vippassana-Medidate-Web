...
export default function AreaTeacherReviewPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
        <div className="mb-8">
          <h1 className="font-serif text-3xl font-bold text-foreground mb-2">Area Teacher Final Review</h1>
          <p className="text-warm-gray text-sm">Provide final approval or rejection for escalated applications.</p>
        </div>
          ...
          <div className="lg:col-span-1 space-y-4 max-h-[80vh] overflow-y-auto custom-scrollbar pr-2">
            {loading ? (
              ...
              <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-8 text-center border border-sand">
                ...
                <p className="text-warm-gray text-sm">No applications pending final review.</p>
              </div>
            ) : (
              reviews.map(review => (
                <button
            ...
            <AnimatePresence mode="wait">
              {selectedReview ? (
                ...
                >
                  ...
                  {selectedReview.reviewerId === user.id && (
                    <div className="space-y-4 border-t border-sand pt-6">
                      <h3 className="font-medium text-foreground">Final Decision</h3>
                      <textarea
                        value={remarks}
                      ...
                      <div className="flex flex-wrap gap-3 pt-2">
                        ...
                        >
                          <CheckCircle size={16} /> Finalize Approval & Enroll
                        </button>
                        <button
                          onClick={() => handleAction("NEEDS_INFO")}
                          disabled={submitting || !remarks.trim()}
                          className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 flex-1 justify-center"
                        >
                          <AlertCircle size={16} /> Request Info
                        </button>
                        <button
                          onClick={() => handleAction("REJECT")}
                          disabled={submitting || !remarks.trim()}
                          className="flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white text-sm font-medium rounded-xl hover:bg-red-700 transition-colors disabled:opacity-50 flex-1 justify-center"
                        >
                ...
                <div className="h-full flex items-center justify-center min-h-[400px] bg-white/40 border border-dashed border-sand/50 rounded-2xl">
                  <p className="text-warm-gray">Select an application to make a final decision.</p>
                </div>
              )}
            </AnimatePresence>
          </div>
